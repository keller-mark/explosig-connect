...
def connect(session_id=None, password=None, server_hostname=SERVER_HOSTNAME, client_hostname=CLIENT_HOSTNAME, how='auto'):
    ...
    if session_id == None:
        conn = EmptyConnection(token, server_hostname, client_hostname)
        conn.open(how=how)
        return conn
    else:
